...
function Board:init(x,y, stats)
    ...
    --instantiate a coin
    local coinRow = math.random(1, Board.MAXROWS)
    local coinCol = math.random(1, Board.MAXCOLS)

    self.tiles = Matrix:new(Board.MAXROWS,Board.MAXCOLS)
    for i=1, Board.MAXROWS do
        for j=1, Board.MAXCOLS do
            --place the coin where specified, tell createGem it is a coin
            if i == coinRow and j == coinCol then
                self.tiles[i][j] = self:createGem(1,3, true)
            else
                self.tiles[i][j] = self:createGem(i,j, false)
            end
        end -- end for j
    end -- end for i
...
function Board:createGem(row,col, coin)
    --if it is a coin, be sure to set the type
    if coin then
...
function Board:findHorizontalMatches()
    ...
    for i = 1, Board.MAXROWS do 
        ...
        for j = 2, Board.MAXCOLS do
            ...
            local currType = self.tiles[i][j].type
            if self.tiles[i][j].type == self.tiles[i][j-1].type or

                --check of the type of this gem is a coin, if so, add it in
                self.tiles[i][j].type <= 3 or
                --if the previous was a coin, and you're of the looked for  type
                (self.tiles[i][j-1].type <=3 and currType == self.tiles[i][j].type) then
...
function Board:findVerticalMatches()
    ...
    for j = 1, Board.MAXCOLS do 
        ...
        for i = 2, Board.MAXROWS do
            ...
            local currType = self.tiles[i][j].type
            if self.tiles[i][j].type == self.tiles[i-1][j].type or 

                --check of the type of this gem is a coin, if so, add it in
                self.tiles[i][j].type <= 3 or
                --if the previous was a coin, and you're of the looked for  type
                (self.tiles[i-1][j].type <=3 and currType == self.tiles[i][j].type) then
...
function Board:matches()
    ...
    if #horMatches > 0 or #verMatches > 0 then -- if there are matches
        for k, match in pairs(horMatches) do
            if coin then
                score = score + 2^match.size * 10 + 50
            else
            ...
            for j=0, match.size-1 do
                ...
                --check if any of the gems in a match are a coin, if so add extra score
                for i=0, match.size-1 do

                    --NIL CHECKING DUE TO BUG
                    if not (self.tiles[match.row][match.col+j] == nil) then
                        local gemType = self.tiles[match.row][match.col+j].type
                        if gemType == 1 or gemType == 2 or gemType == 3 then
                            score = score + 100
                        end
                    end
                end
                ...
                --NIL CHECKING DUE TO BUG
                if not (self.tiles[match.row][match.col+j] == nil) then
                    matchType = self.tiles[match.row][match.col+j].type
                end 
                self.tiles[match.row][match.col+j] = nil
                self:createExplosion(match.row,match.col+j, matchType)
            end -- end for j 
        ...
        for k, match in pairs(verMatches) do
            ...
            for i=0, match.size-1 do
                ...
                for i=0, match.size-1 do
                    ...
                    --NIL CHECKING DUE TO BUG
                    if not (self.tiles[match.row+i][match.col] == nil) then
                        local gemType = self.tiles[match.row+i][match.col].type
                        if gemType == 1 or gemType == 2 or gemType == 3 then
                            score = score + 100
                        end
                    end
                ...
                --NIL CHECKING DUE TO BUG
                if not self.tiles[match.row+i][match.col] == nil then
                    matchType = self.tiles[match.row+i][match.col].type
                end 
                self.tiles[match.row+i][match.col] = nil
                self:createExplosion(match.row+i,match.col, matchType)
            end -- end for i 
        ...
        local prevLevel = stats.level
        self.stats:addScore(score)

        self:shiftGems()
        --check if a level up occured, if it did, make a new coin when u make new gems
        if prevLevel < stats.level then
            self:generateNewGems(true)
        else
            self:generateNewGems(false)
        end
...
function Board:generateNewGems(coin)
    ...
    for j = 1, Board.MAXCOLS do
        ...
        for i = Board.MAXROWS, 1, -1  do -- find an empty space
            if self.tiles[i][j] == nil then -- empty, create new gem & tween 
                if coin then --spawn one coin
                    self.tiles[i][j] = Gem(self.x+(j-1)*Board.TILESIZE,topY, math.random(1,3))
