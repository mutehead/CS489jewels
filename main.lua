...
function love.keypressed(key)
    ...
    elseif key == "return" and gameState=="start" then
        gameState = "play"
    elseif key == "return" and gameState=="over" then           -- restarts game with updated board and scores
        gameState = "start"
        stats = Stats()
        board = Board(140,80,stats)
    end
...
function drawGameOverState()
    ...
    love.graphics.printf("Press Enter to Play or Escape to exit",
        0,90, gameWidth,"center")
    love.graphics.printf("Score: "..tostring(stats.totalScore), 0,130, gameWidth,"center")
    love.graphics.printf("Level: "..tostring(stats.level), 0,170, gameWidth,"center")
