...
function Explosion:init()
    ...
    --self.particleSystem:setColors(1, 0, 0, 1, 0, 0, 0, 0) 
...
function Explosion:trigger(x,y, type)
    ...
    --color setting
    if type == 4 then --yellow
        self:setColor(1,1,0)
    elseif type == 5 then
        self:setColor(0,0,1) --blue
    elseif type == 6 then
        self:setColor(0.3,0.3,0.3) --grey
    elseif type == 7 then
        self:setColor(1,0,0)
    elseif type == 8 then
        self:setColor(0,1,0)
    elseif type == 1 then --gold (yellow)
        self:setColor(1,1,0)
    elseif type == 2 then --silver (grey)
        self:setColor(0.3,0.3,0.3)
    elseif type == 3 then --red coin
        self:setColor(1,0,0)
    end
